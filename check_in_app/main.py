...
def evaluation_matrix(p, res, bottom_line):
    ...
    while True:
        ...
        if res['global_min'] < bottom_line + max_sum:
            return
        p[j][i] = np.inf
        res['local_result'] = res['local_result'][:v_len]
...
# Рисуем всё древо
nx.draw_networkx(graph, points, width=1, edge_color="#C0C0C0")
# Добавляем оптимальный путь
nx.draw_networkx(graph, points, with_labels=True, edgelist=res['best_result'], arrows=True, edge_color="red", width=5)
plt.show()
